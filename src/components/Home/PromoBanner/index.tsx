...
interface Props {
  initialData?: StaticHeroBanner[]
}

// ── Fallback hardcodeado ──────────────────────────────────────────────────────

const LargeBannerFallback = () => (
  <div className="relative z-1 overflow-hidden rounded-2xl bg-dark py-12.5 lg:py-17.5 xl:py-22.5 px-4 sm:px-7.5 lg:px-14 xl:px-19 mb-7.5">
    <div className="max-w-[320px] w-full relative z-10">
  ...
  <div className="grid gap-7.5 grid-cols-1 lg:grid-cols-2">
    <div className="relative z-1 overflow-hidden rounded-2xl bg-gray-2 py-10 xl:py-16 px-4 sm:px-7.5 xl:px-10">
      <div>
        ...
        <h2 className="font-bold text-xl lg:text-heading-4 text-dark mb-2.5">Equipá tu cocina</h2>
    ...
    <div className="relative z-1 overflow-hidden rounded-2xl bg-gray-2 py-10 xl:py-16 px-4 sm:px-7.5 xl:px-10">
      <div>
        ...
        <h2 className="font-bold text-xl lg:text-heading-4 text-dark mb-2.5">Frituras perfectas</h2>
...
const LargeBannerDB = ({ banner }: { banner: StaticHeroBanner }) => (
  <div className="relative z-1 overflow-hidden rounded-2xl bg-dark py-12.5 lg:py-17.5 xl:py-22.5 px-4 sm:px-7.5 lg:px-14 xl:px-19 mb-7.5">
    <div className="max-w-[320px] w-full relative z-10">
      <span className="block mb-3 text-xl font-medium text-blue">
        {banner.bannerName}
      ...
      {banner.product.slug && (
        <Link
          href={banner.product.slug.startsWith('/') ? banner.product.slug : `/${banner.product.slug}`}
          className="inline-flex font-semibold text-custom-sm text-dark bg-blue py-3 px-7 rounded-lg ease-out duration-200 hover:bg-blue-dark mt-7.5"
        >
          Ver producto
        </Link>
    ...
    {banner.bannerImage && (
      <div className="absolute bottom-0 right-4 lg:right-26 -z-1 hidden sm:flex items-end justify-center">
        ...
        <img src={banner.bannerImage} alt={banner.bannerName} className="h-[260px] w-auto object-contain rounded-t-xl" />
      </div>
    )}
  </div>
)

const SmallBannerDB = ({ banner }: { banner: StaticHeroBanner }) => (
  <div className="relative z-1 overflow-hidden rounded-2xl bg-gray-2 py-10 xl:py-16 px-4 sm:px-7.5 xl:px-10">
    {banner.bannerImage && (
      // eslint-disable-next-line @next/next/no-img-element
      <img src={banner.bannerImage} alt={banner.bannerName} className="absolute inset-0 w-full h-full object-cover opacity-10" />
    )}
    <div className="relative">
      <span className="block text-lg text-dark mb-1.5">{banner.bannerName}</span>
      <h2 className="font-bold text-xl lg:text-heading-4 text-dark mb-2.5">{banner.subtitle}</h2>
      {banner.product.slug && (
        <Link
          href={banner.product.slug.startsWith('/') ? banner.product.slug : `/${banner.product.slug}`}
          className="inline-flex font-medium text-custom-sm text-white bg-dark hover:bg-dark-2 py-3 px-7 rounded-lg ease-out duration-200 mt-7.5"
...
const PromoBanner = ({ initialData }: Props) => {
  const useDB = initialData && initialData.length > 0
    ...
    return (
      <section className="py-20 overflow-hidden">
        <div className="w-full px-4 mx-auto max-w-7xl sm:px-8 xl:px-0">
          <LargeBannerFallback />
          <SmallBannersFallback />
        </div>
      </section>
    )
  }

  const [large, small1, small2] = initialData

  return (
    <section className="py-20 overflow-hidden">
      <div className="w-full px-4 mx-auto max-w-7xl sm:px-8 xl:px-0">
        <LargeBannerDB banner={large} />
        {(small1 || small2) && (
          <div className="grid gap-7.5 grid-cols-1 lg:grid-cols-2">
            {small1 && <SmallBannerDB banner={small1} />}
            {small2 && <SmallBannerDB banner={small2} />}
          </div>
        )}
      </div>
    </section>
